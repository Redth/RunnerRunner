...
public class RunnerLifecycleManagerTests
{
    ...
    public async Task StartRunner_TracksInstance()
    {
        ...
        backend.StartRunnerAsync(Arg.Any<RunnerStartRequest>(), Arg.Any<CancellationToken>())
            .Returns(new RunnerInstanceInfo { InstanceHandle = "container-123", RunnerName = "test-runner" });
    ...
    public async Task StopRunner_RemovesFromTracking()
    {
        ...
        backend.StartRunnerAsync(Arg.Any<RunnerStartRequest>(), Arg.Any<CancellationToken>())
            .Returns(new RunnerInstanceInfo { InstanceHandle = "container-456", RunnerName = "runner-1" });
        ...
        await backend.Received(1).StopRunnerAsync("container-456", Arg.Any<CancellationToken>());
    }
